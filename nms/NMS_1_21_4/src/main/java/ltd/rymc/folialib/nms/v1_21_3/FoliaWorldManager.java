package ltd.rymc.folialib.nms.v1_21_3;
...
public class FoliaWorldManager implements WorldManager {
    ...
    private void saveAllChunksNoCheck(ServerLevel world, @NotNull ChunkHolderManager holderManager, final boolean flush, final boolean shutdown, final boolean logProgress, final boolean first, final boolean last) {
        RegionizedServer.ensureGlobalTickThread("Saving all chunks can be done only on global tick thread");
        ...
        if (shutdown) {
            ...
            for (int i = 0, len = holders.size(); i < len; ++i) {
                ...
                ChunkAccess currentChunk = holder.getCurrentChunk();
                if (currentChunk instanceof LevelChunk) {
                    LevelChunk levelChunk = (LevelChunk) currentChunk;
    ...
    public void save(@NotNull ServerLevel level, boolean flush, boolean savingDisabled, boolean close) {
        if (!savingDisabled) {
            Bukkit.getPluginManager().callEvent(new org.bukkit.event.world.WorldSaveEvent(level.getWorld())); // CraftBukkit
    ...
    public boolean unloadWorld(@NotNull World world, boolean save) {
        RegionizedServer.ensureGlobalTickThread("World unload can be done only on global tick thread");
